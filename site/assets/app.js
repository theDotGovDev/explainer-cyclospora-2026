...
/* Search and risk filter for the 100-food table on foods.html.
   Same progressive-enhancement contract as the card grid: with JS off every
   row is visible and the filter UI is hidden by CSS. */
(function () {
  'use strict';

  var body = document.getElementById('t100-body');
  var search = document.getElementById('t100-search');
  if (!body || !search) return;

  var rows = Array.prototype.slice.call(body.querySelectorAll('tr'));
  var chips = Array.prototype.slice.call(document.querySelectorAll('.chip[data-t100]'));
  var status = document.getElementById('t100-status');
  var empty = document.getElementById('t100-empty');
  var band = 'all';
  var query = '';
  ...
  function apply() {
    ...
    rows.forEach(function (row) {
      var okBand = band === 'all' || row.dataset.band === band;
      var okText = !query || row.dataset.name.indexOf(query) !== -1;
    ...
    if (status) {
      status.textContent = shown === rows.length
        ? 'Showing all ' + rows.length + ' foods.'
        : 'Showing ' + shown + ' of ' + rows.length + ' foods.';
    }
  }
